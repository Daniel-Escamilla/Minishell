...
/*   Updated: 2024/09/17 16:09:21 by descamil         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_close_fd(t_mini *mini, int who)
{
	...
	if (who == 'P' && mini->fd_pipe[1] > 0 && mini->fd_pipe[0] != STDOUT_FILENO)
	{
		close(mini->fd_pipe[1]);
		mini->fd_pipe[1] = STDOUT_FILENO;
	}
	if (who == 'P' && mini->fd_pipe[0] > 0 && mini->fd_pipe[0]
		!= STDIN_FILENO && mini->fd_pipe[0] != mini->fd_tmp)
	{
		close(mini->fd_pipe[0]);
		mini->fd_pipe[0] = STDIN_FILENO;
	}
	if (who == 'P' && mini->fd_tmp > 0 && mini->num_comm == 0)
	{
		close(mini->fd_tmp);
		mini->fd_tmp = -1;
	}
